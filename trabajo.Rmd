...
cvKernelSVM <- lapply(folds, function(x){
  ...
  prediccion <- predict(svm(as.factor(Season) ~ ., data = training_fold, kernel = "linear", cost = 10, scale = FALSE),new=training_set)
  y_pred <- predict(clasificador, newdata = test_fold)
  cm <- table(test_fold$Season, y_pred)
  precision <- (cm[1,1] + cm[2,2]) / (cm[1,1] + cm[2,2] +cm[1,2] + cm[2,1])
  return(clasificador)
...
(mc <- with(data,(table(prediccion,Season))))
```
...
Realizamos una primera prueba de K-means con 2 cluster, para ello utilizamos todas las columnas del dataset exceptuando la de Seassons: 
```{r}
summary(km.out) # Inspect the result
...
```
Como se puede ver en la gráfica han salido 3 grupos diferenciados de datos, los cuales se corresponden con la división inicial por Seassons. De esto podemos concluir que la agrupación de k-means ha tenido un resultado satisfactorio, ya que equivale a las 3 Seassons.
...
library("dbscan")
```
Es un método de clusterización adecuado para buscar patrones de agrupación en el espacio físic. Este algorítmo agrupa los puntos que están más cercanos respecto a la distancia euclidiana, además para este algorítmo se tiene que cada cluster contendrá un mínimo de puntos.
...
data <- scale(data)
```
Podemos observar como la columna Seassons tiene 3 cluster a diferencia de las demas columnas.
```{r}
